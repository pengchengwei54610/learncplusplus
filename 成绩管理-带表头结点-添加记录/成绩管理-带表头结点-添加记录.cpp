...
void sort(struct student** head)
{
	...
	{
		for (temp = (*head)->next, count2 = 0; count2 < list_length - count1; temp = temp->next, count2++)
		{
...
Status add_elem(student** head, int newno, int newscore, char* name)
{
	student* temp = (*head)->next;
	student* newnode;
...
//Status change_score(student** head, int queue, int score)
//{
//	student* temp = *head;
//	while (temp != NULL)
//	{
...
int main()
{
	...
	while (1)
	{
		...
		else
		{
			...
			add_elem(&head, query, newscore, newname);
		}
		if (!isContinue())
			break;
	}
